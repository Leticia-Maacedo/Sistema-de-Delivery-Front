...
import { Truck, LogOut, User, Package, Shield } from "lucide-react";
...
import AdminUsuariosView from "./views/cliente/AdminUsuariosView";

import "./styles/global.css";

const ABA_PERFIL = { key: "perfil", label: "Meu Perfil", Icon: User };
const ABA_PRODUTOS = { key: "produtos", label: "Produtos", Icon: Package };
const ABA_USUARIOS = { key: "usuarios", label: "Usuários", Icon: Shield };

// Cada tipo de conta ve so o que faz sentido pra ele: Produtos (cardapio)
// e coisa de Restaurante; Usuarios (gerenciar qualquer conta) e coisa de
// Admin. Cliente e Motoboy ainda nao tem tela propria construida, entao
// ficam so com o Perfil por enquanto.
      ...
      {usuarioLogado ? (
        <>
          ...
          <main style={{ padding: "32px 24px" }}>
            {abaLogada === "produtos" && <ProdutosView />}
            {abaLogada === "usuarios" && <AdminUsuariosView />}
            {abaLogada === "perfil" && <PerfilView aoAtualizarSessao={notificarSessaoAtualizada} />}
          </main>
        </>
      ) : (
